...
constexpr float BATTERY_DIVIDER_RATIO = 24.6f / 3.03f;
    ...
    float knownMass1, float knownMass2)
{
    ...
    Serial.print(F("Tare1 raw = "));
    Serial.println(tareRaw1);
...
void setup()
{
    ...
    while (scale1Busy || scale2Busy)
    {
        ...
        if (scale1Busy)
            Serial.println(F("Scale 1 is busy..."));
        if (scale2Busy)
            Serial.println(F("Scale 2 is busy..."));
        delay(10);
    }
...
void loop()
{
    ...
    if (characterize && !stopped)
    {
        ...
        if (charRampDown && !heldTop && nowUs - lastCharUpdateUs >= 1000000 * topTime)
        { // 0.5 seconds at max throttle for battery comparison
            heldTop = true;
            lastCharUpdateUs = nowUs;
        }
        if (nowUs - lastCharUpdateUs >= 20000)
        { // update every 20ms
            if (!charRampDown)
            {
                lastCharUpdateUs = nowUs;
